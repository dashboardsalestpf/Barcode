...
if desc2 and kategori != "Pilih Kategori":
    ...
    existing_items = st.session_state.master[st.session_state.master['Sub Item'] == subitem]
    if existing_items['ItemCode'].count() == 0:
        # No existing subitem → start with 1
        generate_code = f"{akronim}{num_sub}-{1}"
    else:
        existing_codes = existing_items['ItemCode']
        last_digits = existing_codes.str.split("-").str[-1].astype(int)
        min_digit = last_digits.min()
        max_digit = last_digits.max()
        full_range = set(range(min_digit, max_digit + 1))
        existing_set = set(last_digits)
        missing_numbers = sorted(full_range - existing_set)
        if min_digit != 1:
            generate_code = f"{akronim}{num_sub}-1"
        elif missing_numbers:
            generate_code = f"{akronim}{num_sub}-{missing_numbers[0]}"
        else:
            generate_code = f"{akronim}{num_sub}-{max_digit + 1}"
