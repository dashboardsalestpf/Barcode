...
if search:
    matches = (
        st.session_state.merge.loc[
            st.session_state.merge["ItemCode"].astype(str).str.contains(search, case=False, na=False),
            "ItemCode",
        ]
        .drop_duplicates()
        .sort_values()
...
if itemcodes is not None:
    filtered = st.session_state.merge[
        st.session_state.merge["ItemCode"] == itemcodes
    ]
